...
class CreateParentStudentLinkHandler:
    ...
    def __call__(
        self, command: CreateParentStudentLinkCommand
        ...
        ParentStudentLinkPolicy.ensure_can_create_link(
            actor, parent_id=command.parent_id
        )

        with self._uow_factory() as uow:
            parent_profile = uow.repositories.user_profiles.get(command.parent_id)
            student_profile = uow.repositories.user_profiles.get(command.student_id)
            ...
            if UserRole.PARENT not in parent_profile.roles:
                raise InvariantViolationError(
                    "Связь можно создать только с пользователем роли parent."
                )
            if UserRole.STUDENT not in student_profile.roles:
                raise InvariantViolationError(
                    "Связь можно создать только с пользователем роли student."
                )
            if (
                parent_profile.status != UserStatus.ACTIVE
                or student_profile.status != UserStatus.ACTIVE
            ):
                raise InvariantViolationError(
                    "Связь доступна только для пользователей в статусе active."
                )

            if uow.repositories.parent_student_links.get_active_by_pair(
                command.parent_id,
                command.student_id,
            ):
                raise InvariantViolationError(
                    "Активная связь parent-student уже существует."
                )
...
class RemoveParentStudentLinkHandler:
    ...
    def __call__(
        self, command: RemoveParentStudentLinkCommand
    ) -> ParentStudentLinkResult:
        with self._uow_factory() as uow:
            link = uow.repositories.parent_student_links.get(command.link_id)
            if link is None:
            ...
            ParentStudentLinkPolicy.ensure_can_remove_link(
                actor, parent_id=link.parent_id
            )
...
class ListParentStudentLinksHandler:
    ...
    def __call__(
        self, query: ListParentStudentLinksQuery
    ) -> list[ParentStudentLinkResult]:
        actor = ActorContext.from_claims(query.actor_id, query.actor_roles)
        AdminPolicy.ensure_can_manage_users(actor)
        with self._uow_factory() as uow:
            return [
                to_link_result(link)
                for link in uow.repositories.parent_student_links.list(
